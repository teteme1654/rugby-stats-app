...
- **設定の自動保存**: チーム設定・表示サイズ設定を自動保存し、次回起動時に復元
...
4. **クイックアクション**
   - 「📺 表示画面」「🎥 スコア表示」ボタンでウィンドウの開閉
   - 表示中のボタンは緑色になり「表示中」と表示される
   - 「🔄 試合リセット」で試合データをリセット
...
以下の設定はアプリ終了時も保持され、次回起動時に自動で復元されます：

| 設定項目 | 保存タイミング |
|---|---|
| チーム名・カラー・ロゴ | 変更時 |
| 選手データ | CSV読み込み・選手情報編集時 |
| スタジアム名 | 変更時 |
| 表示サイズ設定 | スライダー操作時 |
| ディスプレイ割り当て | 「適用」ボタン押下時 |
| クロマキー背景色 | 変更時 |

> スコア・統計・交代記録は保存対象外です。試合ごとに「🔄 試合リセット」でリセットしてください。

## トラブルシューティング

### 表示画面が表示されない
- 管理画面のクイックアクションエリアの「📺 表示画面」ボタンをクリックして再表示してください
